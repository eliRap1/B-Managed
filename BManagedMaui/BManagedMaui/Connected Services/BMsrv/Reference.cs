...
// Lets BManagedMaui compile without running the svcutil tool.
//
// ServiceModel does NOT read app.config — Service1Client below configures
// BasicHttpBinding programmatically. The MAUI app builds the client through
// Services/ServiceHelper.GetClient(), which supplies the binding + endpoint.
// ----------------------------------------------------------------------------
...
namespace BManagedMaui.BMsrv
{
    ...
    [ServiceContract(Namespace = "http://tempuri.org/", ConfigurationName = "BMsrv.IService1")]
    ...
    public partial class Service1Client : System.ServiceModel.ClientBase<IService1>, IService1
    {
        ...
        public Service1Client(System.ServiceModel.Channels.Binding binding, EndpointAddress address)
            : base(binding, address) { }

        // --- Task-based wrappers used by the MAUI pages (via ServiceHelper.CallAsync) ---
        // The underlying channel calls are synchronous; Task.Run keeps them off the
        // UI thread so the phone screens stay responsive while the WCF call runs.
        public Task<bool> CheckUserExistAsync(string u)              => Task.Run(() => CheckUserExist(u));
        public Task<bool> CheckUserPasswordAsync(string u, string p) => Task.Run(() => CheckUserPassword(u, p));
        public Task<int>  GetUserIdAsync(string u)                   => Task.Run(() => GetUserId(u));
        public Task<User> GetUserByIdAsync(int id)                   => Task.Run(() => GetUserById(id));
        public Task<OwnerDashboardSnapshot> GetOwnerDashboardSnapshotAsync(int o, string c)
            => Task.Run(() => GetOwnerDashboardSnapshot(o, c));
        public Task<Invoice[]> GetInvoicesForOwnerAsync(int o)       => Task.Run(() => GetInvoicesForOwner(o));
        public Task MarkInvoicePaidForOwnerAsync(int id, int o, DateTime d)
            => Task.Run(() => MarkInvoicePaidForOwner(id, o, d));

        public Task CloseAsync()
        {
            var ico = (System.ServiceModel.ICommunicationObject)this;
            return Task.Factory.FromAsync(ico.BeginClose, ico.EndClose, null);
        }
